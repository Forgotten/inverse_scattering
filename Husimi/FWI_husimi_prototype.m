...
% we will use only 2 Gaussian bumps
MDName = 2;

% radius of support
rn = 0.1; delta_m1 = delta_m;

% center of heterogeneity
centres = [-0.2,-0.2];

% compactly supported heterogeneity
temp = 1 - ((Xi(:)-centres(1)).^2+(Yi(:)-centres(1)).^2)/rn^2;
eta1 = zeros(ni,1);
eta1(temp>0) = delta_m1*exp(1-1./temp(temp>0));

% radius of support of the second bump
rn = 0.2; delta_m2 = 0.3;
...
temp = 1 - ((Xi(:)-centres(1)).^2+(Yi(:)-centres(1)).^2)/rn^2;
eta2 = zeros(ni,1);
eta2(temp>0) = delta_m2*exp(1-1./temp(temp>0));

% adding both bumps together
eta = eta1 + eta2;
...
print(fullfile('..','plots',...
    ['n_reconstruct_medium',int2str(MDName),'_An',num2str(delta_m),'_rn',num2str(rn),...
