...
MDName = 0; %[0,0];                      % shapes of supports (polygon or disk or star polygon)
% radius of support
rn = 0.2; %[0.1,0.2];
...
filter_plot = filter_plot(npml_data+1:npml_data+nyi, npml_data+1:npml_data+nxi);
figure(18); clf();
...
%% Save/load the data
% save(fullfile('..','data',...
%     ['scatter_FD_order',num2str(order_data),...
...
% %     ['n_order',sprintf('-%i',[order_data,order_solver]),'_etai',num2str(alpha_i),'_dNoise',num2str(delta_noise),'_dTol',int2str(-log10(dTol)),...
% %     '_medium',sprintf('-%i',MDName),'_An',sprintf('-%.2f',delta_m_nn),'_rn',sprintf('-%.2f',rn),...
